#---------------------------------------------------------------------
ROOT		= d:\test\tcltk8.3.3
DEVSTUDIO	= c:\progra~1\micros~3

TCLROOT		= $(ROOT)\tcl8.3.3
TCLDIR            = $(TCLROOT)
TKROOT		= $(ROOT)\tk8.3.3

TOOLS32		= $(DEVSTUDIO)\vc98
TOOLS32_rc	= $(DEVSTUDIO)\common\msdev98
TOOLS16		= d:\msvc

INSTALLDIR	= d:\test\Tcl
#---------------------------------------------------------------------
...
winlibs    = $(baselibs) user32.lib gdi32.lib comdlg32.lib winspool.lib imm32.lib
guilibs	   = $(libc) $(winlibs)
conlibs    = $(libc) $(baselibs)
guilibsdll = $(libcdll) $(winlibs)
...
libs	:	libtcltk833.lib $(TMPDIR)\tk.res
	xcopy $(TMPDIR)\tk.res .

libtcltk833.lib:	tcl tk
	set LIB=$(TOOLS32)\lib
	$(link32) -lib $(lflags) \
		-out:$@ \
		$(TCLOBJS) $(TKOBJS)
