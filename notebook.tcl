...
#  $Id: notebook.tcl,v 1.5 1999/05/25 08:28:16 eric Exp $
# ------------------------------------------------------------------------------
#  Index of commands:
#     - NoteBook::create
#     - NoteBook::configure
        ...
        Widget::declare NoteBook::Page {
            ...
            {-text       String     ""     0}
			    ...
			    {-background         TkResource "" 0 button}
 				{-selectedforeground TkResource "" 0 {button -foreground}}
			    {-selectedbackground TkResource "" 0 {button -background}}
...
# ------------------------------------------------------------------------------
proc NoteBook::delete { path pages {destroyframe 1} } {
    variable $path
    upvar 0  $path data
...
proc NoteBook::_highlight { type path page } {
    ...
    switch -- $type {
        on {
            $path:cmd itemconfigure "$page:poly" -fill [Widget::getoption $path.f$page -activebackground]
            $path:cmd itemconfigure "$page:text" -fill [Widget::getoption $path.f$page -activeforeground]
        }
        off {
			if {$data(select) == $page} {
				set bg [Widget::getoption $path.f$page -selectedbackground]
				set fg [Widget::getoption $path.f$page -selectedforeground]
			} else {
				set bg [Widget::getoption $path.f$page -background]
				set fg [Widget::getoption $path.f$page -foreground]
			}
            $path:cmd itemconfigure "$page:poly" -fill $bg
            $path:cmd itemconfigure "$page:text" -fill $fg
        }
    }
}
