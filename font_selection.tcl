namespace eval ::dkfFontSel {
    ...
    proc 'make_UI {w} {
		...
		foreach {but code dir target} {
		    ...
		    pack $b -side top -fill x -padx [expr {$gap/2}] \
			    -pady [expr {$gap/2}]
    ...
    proc dkf_chooseFont {args} {
		...
		#wm transient $w $parent (not working under Windows ...)
...
foreach {pattern value} {
	...
	*DKFChooseFont.DKFChooseFontFrame.relief	groove
	...
	*DKFChooseFont.family.background	gray95
	*DKFChooseFont.family.selectBackground	lightblue
...
if {$tcl_platform(platform) != "windows"} {
	option add *DKFChooseFont.family.selectBackground lightblue
}
if {$tcl_platform(platform) == "windows"} {
	option add *DKFChooseFont.family.selectBackground darkblue
}
