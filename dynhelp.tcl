    ...
    if { [winfo exists $path] } {
        ...
        switch $type {
		   ...
		   canvasballoon {
				...
				if { $text != "" } {
					set _registered($path:$tag) $text
				} else {
					catch {unset _registered($path:$tag)}
				}
				$path bind $tag <Enter>   "DynamicHelp::_motion_balloon enter  $path:$tag %X %Y"
				$path bind $tag <Motion>  "DynamicHelp::_motion_balloon motion $path:$tag %X %Y"
				$path bind $tag <Leave>   "DynamicHelp::_motion_balloon leave  $path:$tag %X %Y"
				$path bind $tag <Button>  "DynamicHelp::_motion_balloon button $path:$tag %X %Y"
